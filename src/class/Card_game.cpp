...
Card_game::Card_game(int min_card_value, int max_card_value ) : number_valued_cards_(
                            compute_number_cards(min_card_value, max_card_value)), min_card_value_(min_card_value),
                                                                max_card_value_(max_card_value), valued_cards_(unique_ptr<unique_ptr<Valued_Card>[]>(new unique_ptr<Valued_Card>[number_valued_cards_]))
{
    create_valued_cards();
}
...
const Valued_Card& Card_game::getCard(size_t card_index) const {
    ...
    if (!valid_card_index)
        throw std::exception();
    return *valued_cards_[card_index];
}

vector<Valued_Card> Card_game::getCards() const {
    vector<Valued_Card> cards;
