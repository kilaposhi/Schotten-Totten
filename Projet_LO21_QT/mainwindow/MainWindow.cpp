...
QString buttonStyle = "QPushButton { background-color: grey; }";
    ...
    :QWidget(parent), player1(player1), player2(player2), clanDeck(clanDeck), tacticDeck(tacticDeck), board(board), game(game)
{
    ...
    QLabel* labelJoueur1 = new QLabel("Score " + QString::fromStdString(player1->getName()) + ": " + QString::number(player1->getScore()));
    QLabel* labelJoueur2 = new QLabel("Score " + QString::fromStdString(player2->getName()) + ": " + QString::number(player1->getScore()));

    QHBoxLayout* score = new QHBoxLayout;
    score->addWidget(labelJoueur1);
    score->addWidget(labelJoueur2);
...
void MainWindow::handleBorderSelected(int index) {
    ...
    if (state==1) {//choisit la border où poser la carte
        ...
        Border& b = board->getBorderByID(index);
        //actualPlayer->claim_borders(b);
        boardV->getBordersV()[index]->getBorderButton()->setStyleSheet(buttonStyle);
        /*QString winner = QString::fromStdString(board->getBorderByID(index).getWinnerBorder()->getName());
        boardV->getBordersV()[index]->getBorderButton()->setText(winner);
        boardV->getBordersV()[index]->getBorderButton()->update();*/
